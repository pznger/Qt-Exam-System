...
ExamPd::ExamPd(QWidget*parent):QDialog(parent)
{
    QFont font;
...
ExamPd::ExamPd(int m,int n,QWidget*parent):
    QDialog(parent)
{
    num=n;
    dayornight=m;
...
void ExamPd::initUI()
{
    ...
    {
        QMessageBox::information(this,"提示","判断只有5套题");
        return;
    }
    for(int i=0;i<10;i++)
    {
...
void ExamPd::getScore()
{
    ...
    if (!wrongSet.isEmpty())
    {
        ...
        QString fileName = QStandardPaths::writableLocation(QStandardPaths::DesktopLocation) + "/" + "判断第"+QString::number(num-1)+"套题目，生成时间："+QDateTime::currentDateTime().toString("yyyyMMdd") + ".txt";   // 在桌面创建一个以时间戳为文件名的文件
        QFile file(fileName);
